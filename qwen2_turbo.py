...
class Qwen2TurboSdpaAttention(Qwen2Attention):
    def forward(
        self,
...
class Qwen2TurboAttention(Qwen2Attention):
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
...
class Qwen2TurboDecoderLayer(Qwen2DecoderLayer):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        super().__init__(config, layer_idx)
        # self.self_attn = Qwen2ModifiedAttention(config, layer_idx)
        self.self_attn = Qwen2TurboSdpaAttention(config, layer_idx)

class Qwen2TurboModel(Qwen2Model):
    def __init__(self, config: Qwen2Config):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [Qwen2TurboDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )

class Qwen2TurboForCausalLM(Qwen2ForCausalLM):
    def __init__(self, config: Qwen2Config):
        super().__init__(config)
        self.model = Qwen2TurboModel(config)